...
if msg_cnt_unack >= max_unack_critical:
    print("CRITICAL: %s - %d unack'd messages." % (queue_name, msg_cnt_unack))
    exit(EXIT_CRITICAL)
elif msg_cnt_unack >= max_unack_warn:
    print("WARN: %s - %d unack'd messages." % (queue_name, msg_cnt_unack))
    exit(EXIT_WARNING)

# /(aqcc.9) Ready to be consumed message count above thresholds
if msg_cnt_ready >= max_ready_critical:
    print("CRITICAL: %s - %d unconsumed messages." % (queue_name, msg_cnt_ready))
    exit(EXIT_CRITICAL)
elif msg_cnt_ready >= max_ready_warn:
    print("WARN: %s - %d unconsumed messages." % (queue_name, msg_cnt_ready))
    exit(EXIT_WARNING)

# /(aqcc.10) Message counts below thresholds, return OK status
print("OK: %s - %d in-flight messages. %dB used memory." % (queue_name, msg_cnt_total, resp_payload["memory"]))
exit(EXIT_OK)

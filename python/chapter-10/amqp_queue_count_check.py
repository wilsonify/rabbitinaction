...
assert 'BlockingConnection' in dir(pika)
...
if response.method.message_count >= max_critical:
    print("CRITICAL: Queue %s message count: %d" % (queue_name, response.method.message_count))
    exit(EXIT_CRITICAL)

# (aqcc.5) Message count is above warning limit
if response.method.message_count >= max_warn:
    print("WARN: Queue %s message count: %d" % (queue_name, response.method.message_count))
    exit(EXIT_WARNING)

# (aqcc.6) Connection OK, return OK status
print("OK: Queue %s message count: %d" % (queue_name, response.method.message_count))
exit(EXIT_OK)
